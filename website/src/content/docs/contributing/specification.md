...
- **Sources:** Garry Tan’s earlier local-memory spec + internal architecture notes and reviews

# Quaid - Personal Knowledge Memory

> Open-source personal knowledge memory. SQLite + FTS5 + vector embeddings in one file. Thin CLI harness, fat skill files. MCP-ready from day one. Runs anywhere. No API keys, no internet, no Docker. Truly static single binary.

Inspired by Garry Tan's earlier local-memory work, with this spec adapting similar goals to a local-first Rust + SQLite architecture intended for portable, offline use.

- **Status:** Spec complete v4 — ready to build core (see [Phased Delivery](#phased-delivery))
- **Repo:** GitHub.com/quaid-app/quaid
- **License (planned):** MIT
- **Origin:** Inspired by Garry Tan's earlier local-memory spec (2026-04-05), then extended with architecture improvements (2026-04-06) and memory research integration (2026-04-08)
...
Git doesn't scale past ~5,000 markdown files. At 7,471 files and 2.3GB, a wiki-memory directory becomes slow to clone, painful to search, and unusable for structured queries. Full-text search requires grep. Semantic search requires an external vector database. Cross-references are just markdown links with no queryable graph.
...
### Why Rust over TypeScript/Bun (the earlier reference stack)

| | Garry's earlier spec (TypeScript/Bun) | This spec (Rust) |
|---|---|---|
...
-- ============================================================
-- knowledge_gaps: queries the memory couldn't answer well
...
$ quaid collection add memory /data/memory/ --db ~/memory.db
...
| `memory_ingest` | Ingest a source document — **single transactional mutation** | `{ content: string, source_type: string, source_ref: string, force?: boolean, pages: Array<{slug, content, expected_version, assertions?, links?, timeline_entries?, tags?}> }` |
...
| `memory_unlink` | Remove cross-reference entirely | `{ from: string, to: string, relationship?: string, page_version: number }` |
...
| `memory_tag` | Add/remove tag | `{ slug: string, tag: string, remove?: boolean, page_version: number }` |
...
| `memory_stats` | Memory statistics (includes contradiction + gap counts) | `{}` |
...
- **`memory_ingest`** is a single server-side transactional mutation. The agent passes all page updates, links, timeline entries, assertions, and tags in one call. The server wraps everything in a SQLite transaction, checks `expected_version` for each page, and either commits all or rolls back all. This eliminates the window where side tables (links, timeline, tags) can desync from page content.
- **All page-scoped mutators** (`memory_link`, `memory_unlink`, `memory_timeline_add`, `memory_tag`, `memory_raw`) require `page_version`. The server verifies it matches before mutating and bumps `pages.version` on success. Mismatch → same conflict error as `memory_put`. No write path bypasses the version check.

### Resources

| Resource | URI | Description |
|----------|-----|-------------|
| Page | `memory://pages/{slug}` | Full page content as markdown |
| Index | `memory://index` | All page slugs grouped by type |

### Prompts

| Prompt | Description |
|--------|-------------|
| `memory_briefing` | Compile a briefing from current memory state |
| `memory_ingest_meeting` | Guide for ingesting a meeting transcript |
...
Importing an existing markdown memory (7,471 files at `/data/memory/`):
...
fn parse_markdown_file(content: &str, file_path: &Path) -> ParsedPage {
    ...
    //    /data/memory/people/pedro-franceschi.md → "people/pedro-franceschi"
    let slug = file_path
...
$ quaid collection add memory /data/memory/ --db ~/memory.db   # attach + initial sync in a scratch DB
```
...
for current state. This prevents the dangerous split-memory where raw export silently
...
$ quaid export --dir /tmp/memory-export/
$ quaid validate --original /data/memory/ --exported /tmp/memory-export/
...
$ quaid export --raw --import-id <ID> --dir /tmp/memory-export-raw/
$ diff -r /data/memory/ /tmp/memory-export-raw/   # should be empty

# --raw without --import-id is an error (prevents accidental stale-byte export)
$ quaid export --raw --dir /tmp/memory-export-raw/
...
name: quaid-ingest
description: |
  Ingest meetings, articles, docs, and conversations into the memory.
...
name: quaid-query
description: |
  Answer questions from the memory using FTS5 + semantic search + structured queries.
...
Say so. "The memory doesn't have info on X" is better than hallucinating.
...
name: quaid-maintain
description: |
  Periodic memory maintenance. Find contradictions, stale info, orphan pages,
...
name: quaid-briefing
description: |
  Compile a daily briefing from memory state plus real-time sources.
...
1. **Calendar** — Today's meetings. For each: pull memory pages for participants using progressive retrieval (`--depth summary`).
...
8. **Recent memory changes** — `quaid list --sort updated` filtered to last 24h.
...
description: |
  ...
  memory only, redacted gaps anonymised before external queries, external gaps
...
3. **Check sensitivity classification and approval before researching.**
   ...
   |-------------|-------------------|------------------------|
   | `internal` (default) | No | Search existing memory pages only (`memory_query`, `memory_search`). No network calls. If the memory can't answer it, leave the gap unresolved and note "requires external research — escalate sensitivity via `memory_gap_approve` to proceed." |
...
4. **Research each gap (per sensitivity rules above).**
   - `internal`: re-query memory with alternate phrasing, check backlinks, scan related pages
...
4. **Ingest findings.** Follow `skills/ingest/SKILL.md` — standard four-tier consolidation.
   ...
   - Existing entity enriched → update via `memory_ingest` with `expected_version`
...
1. **Download new binary to a staging path (NOT directly to the install location):**
   ...
   STAGING_DIR="/tmp/quaid-${TARGET_VERSION}"
   mkdir -p "${STAGING_DIR}"

   # Download binary and checksum file using the release artifact filename
   curl -fsSL "https://github.com/quaid-app/quaid/releases/download/${TARGET_VERSION}/quaid-${PLATFORM}" \
     -o "${STAGING_DIR}/quaid-${PLATFORM}"
   curl -fsSL "https://github.com/quaid-app/quaid/releases/download/${TARGET_VERSION}/quaid-${PLATFORM}.sha256" \
     -o "${STAGING_DIR}/quaid-${PLATFORM}.sha256"
   ```

2. **Verify integrity before installing:**
   ```bash
   # .sha256 files use standard 'hash  filename' format — use --check directly
   (cd "${STAGING_DIR}" && shasum -a 256 --check "quaid-${PLATFORM}.sha256")
...
3. **Preserve the current binary for rollback, then install:**
   ...
   cp "${STAGING_DIR}/quaid-${PLATFORM}" "${INSTALL_PATH}" && chmod +x "${INSTALL_PATH}"
   ```
...
Personal knowledge memory. SQLite + FTS5 + local vector embeddings. One binary.
...
Read skills/ before doing memory operations. They contain all workflow logic.
```
...
curl -fsSL "https://github.com/quaid-app/quaid/releases/download/${VERSION}/quaid-${PLATFORM}" -o "quaid-${PLATFORM}"
curl -fsSL "https://github.com/quaid-app/quaid/releases/download/${VERSION}/quaid-${PLATFORM}.sha256" -o "quaid-${PLATFORM}.sha256"
shasum -a 256 --check "quaid-${PLATFORM}.sha256"
# Option A: install for the current user
mkdir -p "${HOME}/.local/bin"
mv "quaid-${PLATFORM}" "${HOME}/.local/bin/quaid"
chmod +x "${HOME}/.local/bin/quaid"

# Option B: install system-wide (requires root)
sudo install -m 755 "quaid-${PLATFORM}" /usr/local/bin/quaid
```
...
- [ ] `src/commands/stats.rs` — memory statistics (including contradiction count)
...
**Checkpoint:** Full test suite passes. Cross-compiled binaries on GitHub Releases. Round-trip validated against production memory. Contradiction detection runs clean on imported data.
...
**Postgres:** Better for multi-user writes, replication, row-level security. None of those apply here. This is one person's memory. One writer, many readers. SQLite's sweet spot is exactly this workload.

**Qdrant/Chroma/Pinecone:** External services. Require network. Require containers or API keys. A personal memory shouldn't need a sidecar container or a paid API to do semantic search. sqlite-vec gives native cosine similarity in the same file, same connection, same query.
...
A different DB file = a different memory. No application-level complexity.
...
Garry Tan's earlier v0.8.0 local-memory prototype (Apr 2026) moved from Supabase to PGLite — an in-process Postgres that runs in a browser or Node.js via WASM. Same principle as our SQLite choice: zero external dependencies, fully local. Three independent teams in the same week (us with SQLite, Garry with PGLite, @ansubkhan with Fastify/SQLite) converged on local embedded databases for agent memory. The architecture is validated.

| | SQLite (quaid) | PGLite (Garry's earlier prototype) |
|---|---|---|
...
The memory is written by AI agents using the CLI or MCP. Vault-backed workflows now attach directories with `quaid collection add` and stay fresh through `quaid serve` on Unix platforms, while `quaid ingest` and `quaid put` remain the explicit single-file write surfaces.
...
Migrations are tested by importing a fixture memory at each prior schema version and verifying post-migration integrity via `quaid validate --all`.
...
**Non-goals:** quaid does not implement user auth, access control, audit logging, or data classification. It is a single-user tool on a single machine. If the machine is compromised, the memory is compromised.
...
| | Garry's earlier spec (v0.8) | This spec (v4) | MemPalace | agentmemory | Obsidian | Notion |
|---|---|---|---|---|---|---|
...
Does `quaid ingest <file>` attempt to parse entities itself (requiring an LLM call), or is it a pass-through that stores the file and expects the agent (via MCP `memory_ingest` tool) to do the parsing?
...
| Date | Event |
|------|-------|
| 2026-04-05 | Garry Tan drafts an initial v1 memory prototype (TypeScript/Bun, OpenAI embeddings). Inspired by hitting git scaling limits at 7,471 files / 2.3GB. |
...
*This spec is designed to stand alone. Everything needed to build Quaid is above — no prior context required. It is explicitly inspired by Garry Tan's earlier local-memory work while pursuing a Rust + SQLite implementation with different deployment trade-offs. v4 integrates memory research from MemPalace, OMNIMEM, and agentmemory, plus community research and Garry Tan's v0.8.0 skillpack analysis. Architecture additions: knowledge gap detection, graph traversal, source attribution standards, filing disambiguation, and three new skills (alerts, research, upgrade).*
