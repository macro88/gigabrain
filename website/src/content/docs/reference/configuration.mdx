---
title: Configuration
description: Environment variables, the memory.db config table, and the ~/.quaid/ filesystem layout.
guide:
  eyebrow: Reference
  accentBar: true
---

Quaid reads configuration from three places, in order of precedence: command-line flags, environment variables, and the `config` table inside `memory.db`. Anything that requires a fresh model or a new database (most notably the embedding model) is also recorded in the immutable `brain_config` table at `init` time.
...
| -------- | ------- | ------- | ----- |
| `QUAID_DB` | every command | `./memory.db` | Path to the memory database file. |
...
The memory itself lives wherever you tell it to — typical defaults are `./memory.db` for project-scoped memories and `~/memory.db` for a personal memory. SQLite WAL sidecars (`memory.db-wal`, `memory.db-shm`) are created at runtime and folded back in by `quaid compact`.
