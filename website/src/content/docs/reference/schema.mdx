...
The current schema version is **`5`**, recorded in both `config` and `brain_config`. Both `quaid validate` and the runtime `open` path verify the version on every load.
