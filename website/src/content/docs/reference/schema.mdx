---
title: Schema
description: User-relevant tables in brain.db — what they store and when they're written.
guide:
  eyebrow: Reference
  accentBar: true
---

`brain.db` is a single SQLite file that holds every page, link, embedding, and audit row. The schema lives in [`src/schema.sql`](https://github.com/macro88/gigabrain/blob/main/src/schema.sql) and is embedded into the binary via `include_str!()`. This page documents the user-relevant tables — internal scaffolding (test guards, watcher state, IPC heartbeats) is excluded.

The current schema version is **`5`**, recorded in both `config` and `brain_config`. Both `gbrain validate` and the runtime `open` path verify the version on every load.
...
Structured timeline rows used by `gbrain timeline-add`. Unique per `(page_id, date, summary_hash)` to make ingestion idempotent.
...
| `content_hash` | TEXT | SHA-256, used for stale detection by `gbrain embed --stale`. |
...
| `source_kind` | TEXT | `wiki_link` (parsed from markdown) or `programmatic` (`brain_link` etc.). |
| `valid_from` / `valid_until` | TEXT (nullable) | Open intervals encode "still active". |
| `created_at` | TEXT | |

### `assertions`

Subject-predicate-object triples used by `brain_check` for contradiction detection.
...
Original byte-exact contents of imported files. Used by `gbrain export --raw` to round-trip through the database without lossy normalization.

### `ingest_log`

Per-file SHA-256 audit trail for `gbrain import` / `gbrain ingest` idempotency. Re-running on the same source content is a no-op unless `--force` is supplied.
...
Materialized contradictions detected by `brain_check`. Cleared on resolution.
...
Mutable runtime defaults editable via `gbrain config`.
