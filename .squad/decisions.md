# Decision: Retroactive OpenSpec for already-released changes

**Date:** 2026-05-02T18:31:04.840+08:00  
**Author:** Leela  
**Context:** Issue #137 / PR #141 / v0.16.0 — per-namespace memory isolation was implemented and released without a prior OpenSpec change

## Decision

When a change is shipped and released before an OpenSpec is created, we create a retroactive OpenSpec change that documents the landed reality. The artifacts are written to reflect what was actually built, not a future plan.

### Specific conventions for retroactive changes

1. **proposal.md** — Include a `**Status:** complete — shipped in vX.Y.Z (PR #NNN, closes issue #NNN)` line immediately after the leading summary. This makes the retrospective nature explicit to any reader.

2. **tasks.md** — All tasks are pre-checked (`- [x]`) because the work is already done. Tasks are written in past-tense-compatible form (they describe what was built, not what to build). The `openspec status` output will correctly report `isComplete: true`.

3. **Follow-up fixes** — Bug fixes and correctness patches that were bundled into the same PR (even if added in later commits) are included as a dedicated section in `tasks.md` (e.g., "## 8. Backward-compatibility fixes"). This preserves the audit trail for why certain edge-case behaviors were added.

4. **Spec files** — Written in the normal ADDED/MODIFIED/REMOVED format. Scenarios reflect the actual shipped contracts, including backward-compatibility guarantees.

5. **Change name** — Derived from the feature slug used in the implementing branch (`feat/namespace-isolation-137` → `namespace-isolation`).

## Rationale

Retroactive OpenSpecs serve as institutional memory and audit trail for changes that bypassed the normal proposal-first workflow. They also enable the archive workflow to function correctly at release time. The pre-checked tasks format avoids misleading any future `openspec apply` run into re-implementing already-shipped work.

## Applied to

- `openspec/changes/namespace-isolation/` — retroactive spec for PR #141 / v0.16.0
