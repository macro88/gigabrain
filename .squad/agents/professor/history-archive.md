...
- Vault-sync Batch 2 final release gate (2026-04-28T21:46:33.929+08:00): **REJECT FOR RELEASE** if a schema bump is not self-consistent inside the embedded DDL. `src/schema.sql` must seed `config.version` to the same `SCHEMA_VERSION` that `src/core/db.rs` enforces, otherwise a crash between schema bootstrap and legacy-config repair can strand a freshly initialized DB in a self-rejecting “legacy” state.
- Vault-sync Batch 2 final release gate (2026-04-28T21:46:33.929+08:00): when an observability task promises `queue_depth` plus `failing_jobs`, hiding `failing_jobs` behind `#[serde(skip_serializing)]` on both MCP and CLI JSON does not count as “contract-preserving”; it is task drift. Reopen the task or surface the field on the promised machine-readable surfaces before marking the batch complete.
- Release-lane prep note (2026-04-28T21:46:33.929+08:00): if the release workflow links a versioned install doc, that doc becomes part of the release artifact and must be updated in the same lane. `release.yml` now points at `docs/getting-started.md`, so stale `v0.10.0` / Batch 1 copy there is a real ship blocker, not a docs nit.

## Learnings

- 2026-04-28T21:46:33.929+08:00 — Batch 2 rereview: schema-seed parity alone is not enough for a breaking SQLite bump. In Quaid, `src/core/db.rs` must also reopen a freshly bootstrapped DB whose `quaid_config` table is still empty after a crash window; otherwise `preflight_existing_schema()` can pass while `open_with_model()` still self-rejects the same DB.
- 2026-04-28T21:46:33.929+08:00 — `memory_collections` remains a frozen 13-field MCP contract, enforced by exact-key tests in `src/mcp/server.rs`. Batch-local diagnostics such as `failing_jobs` should ship via `quaid collection info` unless OpenSpec explicitly reopens the MCP schema.
- 2026-04-28T21:46:33.929+08:00 — Release-truth review for `v0.11.0` spans `README.md`, `docs/getting-started.md`, `docs/roadmap.md`, and `website/src/content/docs/tutorials/install.mdx`. If any linked install/status doc still overclaims the unpublished tag, it remains a release blocker even when code fixes are correct.
- 2026-04-28T21:46:33.929+08:00 — Final release gates must include the repo’s actual CI lint contract, not just behavioral/task closure. For Quaid that means `cargo clippy --all-targets -- -D warnings` staying green; a new Clippy failure in touched Batch 2 code is release-blocking even when tests pass and coverage clears 90%.
...
- Vault-sync Batch 2 pre-review (2026-04-28): Batch 2 is only landable if Fry resolves three contract seams before code lands: (1) the schema bump updates `src/schema.sql`, `src/core/db.rs` `SCHEMA_VERSION`, and every hard-coded version test together; (2) `embedding_jobs` picks one durable unit-of-work contract (per-page vs per-chunk) and a real retry/backoff/resume model instead of mixing `chunk_index` with the current one-row-per-page queue; and (3) collection observability stays honest about the frozen 13-field `memory_collections` schema — CLI may widen, MCP may not unless design/spec/tests are explicitly reopened. Audit every surviving enqueue path (`memory_put`, reconciler, quarantine restore, temporary import shims) or semantic indexing will silently drift behind FTS on compatibility paths.
- PR #110 approval review (2026-04-28): once the direct-push detector proves `merge_commit_sha == github.sha` on a merged PR to `main`, the earlier association-bypass rejection is cleared. A follow-up mechanical regression can still be closed by the minimal borrowed-path fix (`Path::new(...)` instead of `PathBuf::from(...)`), after which local `cargo clippy --all-targets -- -D warnings` and the GitHub `Check` job are sufficient to re-open approval; remaining blockers are broader CI completion and any out-of-band repo settings work, not code design debt.
- PR #110 main-guardrails final review (2026-04-28): a prior scope-rejection can be cleared by narrowing the diff to mechanical gate debt, but the artifact is still rejectable when the GitHub-side direct-push detector treats **any** PR association on `github.sha` as success. For a protected-branch bypass catch, success must prove the commit arrived on `main` via a merged PR targeting `main`, not merely that the commit appeared on some PR branch. Enforce reviewer rotation across each failed revision cycle: Fry locked out, then Mom, then Zapp; next revision owner must be Leela.
- Vault-sync Batch 1 watcher-reliability pre-gate (2026-04-28): **REJECT Batch 1 closure as currently scoped** when the overflow recovery authorization bypass tries to escape `ActiveLease`, the `memory_collections` MCP schema tries to widen past the frozen 13.6 contract, or `WatcherMode` enum contradictions make variants unreachable. Truthful narrowing: overflow recovery mode to `FullHashReconcileMode`, authorization stays `ActiveLease`; watcher health CLI-only on `quaid collection info`; `memory_collections` unchanged. Fry lockout enforced; Mom assigned as implementer for repaired scope.
- Vault-sync Batch 1 pre-gate (2026-04-28): **REJECT Batch 1 closure as currently scoped** when a watcher-reliability slice tries to widen `memory_collections` past the frozen 13-field 13.6 contract. `src/core/reconciler.rs` proves full-hash authority through `FullHashReconcileMode` + `FullHashReconcileAuthorization`; overflow recovery must stay an `ActiveLease`-authorized active-collection path, while watcher health can widen CLI `collection info` but not MCP `memory_collections` unless the spec/test contract is explicitly reopened (`openspec/changes/vault-sync-engine/design.md`, `src/core/reconciler.rs`, `src/mcp/server.rs`, `src/commands/collection.rs`).
