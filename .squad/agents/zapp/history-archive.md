...
- **v0.11.0 release-lane audit (2026-04-28T21:46:33.929+08:00):** Quaid release mechanics are repo-native: `Cargo.toml` is the shipping version source, `src/commands/version.rs` prints `CARGO_PKG_VERSION`, `.github/workflows/release.yml` blocks tag/version mismatch, and `.github/release-assets.txt` is the canonical asset manifest consumed by workflow/tests. The evergreen release-cut surfaces are `Cargo.toml`, `Cargo.lock`, `README.md`, `docs/getting-started.md`, `docs/roadmap.md`, `.github/workflows/release.yml`, `.github/RELEASE_CHECKLIST.md`, `.github/release-assets.txt`, `scripts/install.sh`, and pinned website docs under `website/src/content/docs/`.
- **Roadmap Batch 2 truth split (2026-04-28T21:46:33.929+08:00):** For the `v0.11.0` lane, `docs/roadmap.md` must preserve the frozen-MCP contract: `memory_collections` stays at the 13-field schema and only surfaces `embedding_queue_depth`; failed-job reporting belongs to `quaid collection info` only. The truth anchors for this copy are `openspec/changes/vault-sync-engine/implementation_plan.md`, `openspec/changes/vault-sync-engine/tasks.md`, `src/mcp/server.rs`, and `src/commands/collection.rs`.
...
- [2026-04-29T06-55-46Z] CI/merge lane retry initiated (PR #114 runner failure)
