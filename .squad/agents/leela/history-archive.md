...
### 2026-04-28T21:46:33.929+08:00 — Batch 2 revision

- **Schema bump parity rule:** Quaid has three version authorities that must move together on every schema bump: `src/schema.sql` seeds `config.version`, `src/core/db.rs` enforces `SCHEMA_VERSION`, and `set_version()` writes `PRAGMA user_version`. If any one lags, `preflight_existing_schema()` can misclassify a crash-partial fresh DB as legacy.
- **Frozen MCP contract rule:** `memory_collections` remains the frozen 13-field MCP surface from task 13.6. Batch-local diagnostics like failed embedding jobs must go to `quaid collection info` unless the OpenSpec explicitly reopens the MCP schema and the exact-key test in `src/mcp/server.rs`.
- **CLI JSON is a public surface:** If a field is hidden with `#[serde(skip_serializing)]`, the feature is not truthfully surfaced for `--json` users even when plain-text output shows it. Treat CLI JSON and plain-text as separate public contracts that both need review.
- **User preference / release discipline:** macro88 wants task state kept brutally honest for release gating — fix the code or reopen the task, but never leave a false green checkbox for review.

**Key file paths:**
- `src/schema.sql` — embedded DDL; `config.version` seed must match the runtime schema constant
- `src/core/db.rs` — schema preflight and version refusal path
- `src/commands/collection.rs` — `quaid collection info` public CLI/JSON diagnostics
- `tests/collection_cli_truth.rs` — CLI truth tests for `collection info --json`
- `openspec/changes/vault-sync-engine/implementation_plan.md` — accepted Batch 2 task wording
- `openspec/changes/vault-sync-engine/tasks.md` — release gate source of truth for task closure
