...
- 2026-05-04T07:22:12.881+08:00 — **Semantic-target race truth rule:** On rename-before-commit write paths, a destination-slug mutex is not enough for add-only supersede chains. If two new slugs can contend for the same head, the truthful fix is to claim or serialize on the supersede target before any bytes land; otherwise leave the task open instead of narrowing the contract after the race is discovered.
- 2026-05-04T07:22:12.881+08:00 — **OpenSpec landed-slice reframing:** When the first schema slice has already landed, rewrite completed tasks from future-tense ("add", "bump") to landed-baseline language ("keep", "align", "verify") and add a truth note that remaining work starts at the next open task. This preserves checkbox history without lying about a pending schema bump.
- 2026-05-04T07:22:12.881+08:00 — **OpenSpec schema-version collision gate:** Before batching a schema change, verify the target version against both `src/core/db.rs` and `src/schema.sql`. If the spec's target version is already shipped, stop and truth-repair proposal/design/specs/tasks before more code or checkbox progress continues, or tests will pass against the wrong baseline.
- 2026-05-04T07:22:12.881+08:00 — **DDL vocabulary check:** When OpenSpec DDL refers to a logical page `kind`, confirm the physical column name in the live schema before routing implementation. If the repo still stores that value under `pages.type`, correct the artifacts first; otherwise the first index/task slice is scoped against nonexistent SQL.
