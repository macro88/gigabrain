...
- 2026-05-04T07:22:12.881+08:00 — Conversation-memory slice 1 already has the honest schema proofs in `src\core\db.rs`: fresh-v8 artefact/default checks, `superseded_by` FK enforcement, invalid `extraction_queue` CHECK-path tests, and explicit v7-open rejection. The immediate test debt after `Page.superseded_by` lands is fixture sync plus one legacy-serde proof that missing `superseded_by` still deserializes as `None`; on this branch the trustworthy full validation lane is serial (`RUST_TEST_THREADS=1 cargo test --quiet -j 1` before serial `cargo llvm-cov`) because `commands::embed::tests::run_embeds_chunks_for_all_pages` can fail under default test-thread ordering.
