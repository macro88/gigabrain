...
- Release-branch docs must distinguish the branch target (`v0.11.0` here) from the latest published tag. For GitHub Releases and installer snippets, prefer `<published-tag>` placeholders or explicit "latest published tag" wording until the workflow actually creates the release.
- Batch 2 for the vault-sync line is user-visible even without new commands: schema v7 adds embedding-job state / retry metadata, `quaid serve` drains the queue in the background, and collection status now exposes `queue_depth` plus `failing_jobs`.
- Key release-truth files for this lane: `docs/getting-started.md`, `README.md`, `website/src/content/docs/tutorials/install.mdx`, and `docs/roadmap.md`.
