...
- PR #66 review fix (2026-04-18): Fixed two review findings on model-selection branch. (1) `early_command()` now ORs global and subcommand `json` flags (`cli.json || *json`) so `gbrain --json model list` works correctly. (2) `BrainConfig::to_model_config()` alias matches now include `"medium"` and `"max"` so brain_config round-trip works for all standard aliases. Both fixes verified clean with cargo build/test/clippy.
