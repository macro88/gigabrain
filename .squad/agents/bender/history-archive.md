## 2026-04-28 Clippy Blocker Fix — release/v0.11.0

- **Scope:** Lint gate repair only. No behavior change, no OpenSpec truth changes.
- **Blocker:** `cargo clippy --all-targets -- -D warnings` failed with `clippy::type_complexity` at `src/core/vault_sync.rs:3347` — return type `Result<Vec<(i64, i64, String, i64, i64)>, VaultSyncError>` on `load_embedding_job_candidates`.
- **Fix:** Introduced `type EmbeddingJobRow = (i64, i64, String, i64, i64)` alias immediately before the function. Behavior is identical — no logic touched.
- **Gate result:** `cargo clippy --all-targets -- -D warnings` exits 0. One error, one alias, done.
- **Commit:** `e42b7b3` on `release/v0.11.0`. Pre-existing unstaged vault_sync.rs changes included (per task rule: do not revert unrelated branch changes).

## Learnings

- **`clippy::type_complexity` fires on `Result<Vec<TUPLE_5>, Error>` in Rust.** A 5-field tuple inside `Vec` inside `Result` is enough to cross the complexity threshold. The correct fix is a named `type` alias, not `#[allow(...)]`. Keep aliases local (module scope) to the function they serve.
