...
- Batch 13.6 + 17.5ddd review (2026-04-24): **REJECT.** The new `brain_collections` surface gets the masking and most status shaping right, but it is not trust-boundary safe yet: `integrity_blocked="manifest_incomplete_escalated"` is driven by a hardcoded 30-second threshold instead of the spec's `GBRAIN_MANIFEST_INCOMPLETE_ESCALATION_SECS` default 1800s / 30 min, so agents would be told to stop and reset far too early. Scope honesty is also soft: the MCP response now surfaces the stateful `.gbrainignore` absence discriminator (`file_stably_absent_but_clear_not_confirmed`), which reintroduces deferred `17.5aa5` semantics into a slice that was supposed to stay limited to 13.6 + exact-schema closure.
- Batch 13.3 CLI slug-parity review (2026-04-24): **REJECT.** Most direct slug-bearing CLI surfaces now resolve explicit `<collection>::<slug>` correctly and emit canonical page addresses, and the `assertions.rs` UUID-first lookup is a necessary safety repair because `brain_check` / CLI `check` now operate in duplicate-slug brains where bare-slug `page_id` lookup would overwrite the wrong page's import assertions. The blocking seam is fail-closed truth: `hybrid_search_canonical()` still turns an ambiguous exact-slug query into `Ok(None)` and silently falls through to generic search/no-results semantics instead of surfacing the same ambiguity boundary, and `gbrain unlink` still reports raw user inputs after both endpoints were already resolved collection-aware.
- Batch N1 MCP slug-routing truth review (2026-04-24): **REJECT.** The MCP-facing slug resolution and canonical response surfaces look adversarially sound for the intended N1 slice: ambiguous bare slugs fail closed with stable `{ code: "ambiguous_slug", candidates: [...] }`, explicit `<collection>::<slug>` resolves through `parse_slug`, and MCP outputs that reference pages are canonically qualified. The blocking seam is honest-scope, not trust-boundary correctness: `src/commands/check.rs` now changes the CLI `check` path to resolve explicit collection slugs by page-id, which is partial deferred `13.3` CLI parity leaking through a shared helper. Smallest acceptable repair is to keep the MCP routing fix without widening the CLI contract or, at minimum, stop counting that widened CLI behavior as part of N1.
...
## 2026-04-24 13.6 + 17.5ddd Review

- **Verdict:** REJECT. `src/core/vault_sync.rs::parse_ignore_parse_errors()` now strips every non-`parse_error` arm, and `src/mcp/server.rs::brain_collections_surfaces_status_flags_and_terminal_precedence()` asserts that `file_stably_absent_but_clear_not_confirmed` is hidden as `null`. That contradicts the frozen `brain_collections` schema still documented in `openspec\changes\vault-sync-engine\design.md` § `brain_collections`, which explicitly includes both tagged-union codes. So this slice does not truthfully close exact schema fidelity yet.

## 2026-04-24 13.6 + 17.5ddd Re-review
