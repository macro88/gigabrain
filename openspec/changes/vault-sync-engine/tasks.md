...
- [ ] 2.4c Walks use `walk_to_parent` then iterate entries via `readdir` with `stat_at_nofollow`, skipping symlinks with WARN. Never descend into symlinked directories.
  > **Primitives ready; integration deferred:** `walk_to_parent` and `stat_at_nofollow` are implemented in `fs_safety.rs` with correct O_NOFOLLOW / AT_SYMLINK_NOFOLLOW semantics. The actual readdir iteration, per-entry `stat_at_nofollow` call, and WARN-on-symlink skip logic have not been wired into any walk yet — that is what task 5.2 (`ignore::WalkBuilder` reconciler integration) actually requires.
