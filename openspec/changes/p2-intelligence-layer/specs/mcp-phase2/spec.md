...
### Requirement: brain_link MCP tool
`src/mcp/server.rs` SHALL expose a `brain_link` tool that creates a typed temporal link
...
- **WHEN** `brain_link` is called with `from_slug="people/alice"`, `to_slug="companies/acme"`,
...
- **WHEN** `brain_link` is called with `from_slug="people/ghost"` which does not exist
- **THEN** the tool returns `ErrorCode(-32001)` with "page not found" in the message

### Requirement: brain_link_close MCP tool
`src/mcp/server.rs` SHALL expose a `brain_link_close` tool that closes a link by ID.
...
- **WHEN** `brain_link_close` is called with a valid `link_id` and `valid_until="2025-06"`
- **THEN** the link's `valid_until` is updated and the tool returns confirmation

#### Scenario: Unknown link_id returns not-found error
- **WHEN** `brain_link_close` is called with `link_id=99999`
- **THEN** the tool returns `ErrorCode(-32001)` with "link not found" in the message

### Requirement: brain_backlinks MCP tool
`src/mcp/server.rs` SHALL expose a `brain_backlinks` tool that returns inbound links for a page.
...
- **WHEN** `brain_backlinks` is called with `slug="companies/acme"`
...
- **WHEN** `brain_backlinks` is called with `slug="nobody/ghost"`
- **THEN** the tool returns `ErrorCode(-32001)`

### Requirement: brain_graph MCP tool
`src/mcp/server.rs` SHALL expose a `brain_graph` tool that returns the N-hop neighbourhood
...
- **WHEN** `brain_graph` is called with `slug="people/alice"`, `depth=2`
- **THEN** the tool returns a JSON object with `nodes` and `edges` arrays

#### Scenario: Unknown slug returns not-found error
- **WHEN** `brain_graph` is called with `slug="people/ghost"`
- **THEN** the tool returns `ErrorCode(-32001)`

### Requirement: brain_check MCP tool
`src/mcp/server.rs` SHALL expose a `brain_check` tool that detects contradictions.
...
- **WHEN** `brain_check` is called with `slug="people/alice"` and there are contradictions
- **THEN** the tool returns a JSON array of contradiction objects

#### Scenario: Clean page returns empty array
- **WHEN** `brain_check` is called with `slug="people/alice"` and no contradictions exist
- **THEN** the tool returns an empty JSON array `[]`

### Requirement: brain_timeline MCP tool
`src/mcp/server.rs` SHALL expose a `brain_timeline` tool that reads timeline entries for a page.
...
- **WHEN** `brain_timeline` is called with `slug="people/alice"` and timeline entries exist
- **THEN** the tool returns a JSON array of entry strings ordered by date DESC

#### Scenario: Unknown slug returns not-found error
- **WHEN** `brain_timeline` is called with `slug="nobody/ghost"`
- **THEN** the tool returns `ErrorCode(-32001)`

### Requirement: brain_tags MCP tool
`src/mcp/server.rs` SHALL expose a `brain_tags` tool that reads, adds, or removes tags.
...
- **WHEN** `brain_tags` is called with `slug="people/alice"` and no add/remove
- **THEN** the tool returns a JSON array of current tag strings

#### Scenario: Add a tag
- **WHEN** `brain_tags` is called with `slug="people/alice"`, `add=["investor"]`
- **THEN** "investor" is inserted; the tool returns the updated tag list

#### Scenario: Remove a tag
- **WHEN** `brain_tags` is called with `slug="people/alice"`, `remove=["investor"]`
- **THEN** "investor" is deleted; the tool returns the updated tag list (without "investor")

#### Scenario: Unknown slug returns not-found error
- **WHEN** `brain_tags` is called with `slug="nobody/ghost"`
- **THEN** the tool returns `ErrorCode(-32001)`
